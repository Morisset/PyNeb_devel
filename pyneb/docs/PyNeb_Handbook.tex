...
\chapter{PyNeb's Handbook}\label{pynebs-handbook}
...
\subsection{Installing the code with
pip}\label{installing-the-code-with-pip}
...
\subsection{Installing the code from the tar
file}\label{installing-the-code-from-the-tar-file}
...
\section{First steps using PyNeb}\label{first-steps-using-pyneb}
...
\section{How atoms are represented: the Atom and RecAtom
classes}\label{how-atoms-are-represented-the-atom-and-recatom-classes}
...
\subsection{The Atom class}\label{the-atom-class}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/core/pynebcore.py:1011: VisibleDeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
  self.\_CollArray[lev\_i-1, lev\_j-1, :] = coll\_data[i,2:]
...
\subsection{Print and plot atomic
properties}\label{print-and-plot-atomic-properties}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/numpy/core/numeric.py:190: VisibleDeprecationWarning: using a non-integer number instead of an integer will result in an error in the future
...
\subsection{Using Atom to compute populations and
emissivities}\label{using-atom-to-compute-populations-and-emissivities}
...
\subsection{Physical conditions determined from line
ratios}\label{physical-conditions-determined-from-line-ratios}
...
\subsection{Ionic abundance
determination}\label{ionic-abundance-determination}
...
\subsection{Creating a dictionary of Atom
objects}\label{creating-a-dictionary-of-atom-objects}
...
conventional atom names \(<\)element\(><\)spectrum\(>\), and the
corresponding entries the atoms themselves; e. g.:
...
\section{The atomic data}\label{the-atomic-data}

\subsection{Changing the data files used for atomic
data}\label{changing-the-data-files-used-for-atomic-data}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/../atomic\_data/
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/../atomic\_data/
...
\subsection{Predefined sets of atomic
data}\label{predefined-sets-of-atomic-data}
...
\subsection{Creating new atomic data
files}\label{creating-new-atomic-data-files}
...
\subsection{Plotting atomic data}\label{plotting-atomic-data}
...
warng \_ManageAtomicData: trc data not available for O3
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:165: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
  ttype=Splups["ttype"][isplups]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:166: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
  cups=Splups["cups"][isplups]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:168: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
  nspl=Splups["ntemp"][isplups]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:170: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
  xs = Splups['btemp'][isplups]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:171: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
  splups=Splups["bscups"][isplups]
/Users/christophemorisset/anaconda/lib/python2.7/site-packages/pyneb/utils/pn\_chianti.py:177: VisibleDeprecationWarning: converting an array with ndim > 0 to an index will result in an error in the future
...
\subsection{Recombination lines: the RecAtom
class}\label{recombination-lines-the-recatom-class}
...
Enter \textbf{\(<\)Atom.labels\(>\)} to display the valid labels for
both kind of recombiation atoms, i.e.:
...
\end{Verbatim}
            ...
            \begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}88}]:} <matplotlib.colorbar.Colorbar at 0x11356f410>
\end{Verbatim}
...
\section{The extinction class:
RedCorr()}\label{the-extinction-class-redcorr}

The class RedCorr manages the extinction (reddening) correction. It can
compute the logarithmic extinction at H\(\beta\) by comparing an
observed ratio to a theoretical one (usually H\(\alpha\)/H\(\beta\), but
any other ratio can be used). The object is also able to compute the
correction to be applied to any intensity, given the wavelength of the
line.
...
\[(1-f_\lambda).c(H\beta) = 0.4 E_{B-V} X_\lambda\] applied to
\(\lambda\) = 4861, with \(f_\beta\) = 0.:
...
and the correction relative to any other wavelength (p.~ej.,
H\(\alpha\)) is given by:
...
\section{The EmissionLine class}\label{the-emissionline-class}
...
\section{The Observation class: reading and dealing with
observations}\label{the-observation-class-reading-and-dealing-with-observations}

\subsection{Reading observation from a
file}\label{reading-observation-from-a-file}
...
\subsection{Adding observations and
lines}\label{adding-observations-and-lines}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
S4\_10.5m [  1.86287333e+06   2.78459278e-01]
Ne2\_12.8m [  2.28962464e+06   9.53623136e-01]
Ne3\_15.6m [  9.68184283e+06   5.60162375e-01]
S3\_18.7m [  2.90091821e+06   5.36780528e-01]
O2\_3726A [ 0.99069915  0.67691353]
O2\_3729A [ 0.46892362  0.01428534]
Ne3\_3869A [ 0.75888083  0.96276666]
Ne3\_3968A [ 0.35320246  0.13828883]
S2\_4069A [ 0.06443513  0.46249405]
S2\_4076A [ 0.03486379  0.99805526]
O3\_4363A [ 0.81982369  0.59713807]
H1r\_4861A [ 100.            0.78223163]
O3\_5007A [  7.24228868e+02   1.58710251e-01]
N2\_5755A [ 7.55158652  0.40264241]
S3\_6312A [ 38.21722558   0.40319109]
O1\_6300A [ 82.92131172   0.66824492]
O1\_6364A [ 30.17724087   0.95694954]
N2\_6548A [  5.49035398e+02   2.52928787e-01]
H1r\_6563A [ 285.            0.70501826]
N2\_6584A [  1.63368483e+03   8.85168908e-01]
S2\_6716A [ 134.21899293    0.4385847 ]
S2\_6731A [ 246.50683125    0.88167321]
Ar3\_7136A [  1.11657610e+03   3.88988790e-01]
O2\_7319A+ [  1.98764418e+03   8.53219275e-01]
O2\_7330A+ [  1.59812656e+03   8.98304031e-01]
...
\subsection{Getting line intensities}\label{getting-line-intensities}
...
\end{Verbatim}
            ...
            \begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}139}]:} array([  1.59812656e+03,   8.98304031e-01])
\end{Verbatim}
        
\section{Using Observation to determine ionic
abundances}\label{using-observation-to-determine-ionic-abundances}
...
\section{Exploring line intensities with the EmisGrid
class}\label{exploring-line-intensities-with-the-emisgrid-class}

\subsection{The EmisGrid class}\label{the-emisgrid-class}
...
\subsection{Instantiating various EmisGrid objects with
getEmisGridDict}\label{instantiating-various-emisgrid-objects-with-getemisgriddict}
...
\end{Verbatim}
            ...
            \begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}150}]:} \{'N2': <pyneb.core.emisGrid.EmisGrid at 0x115c51150>,
           'O2': <pyneb.core.emisGrid.EmisGrid at 0x117767e10>,
           'O3': <pyneb.core.emisGrid.EmisGrid at 0x117767490>\}
\end{Verbatim}
...
\section{The Diagnostics class}\label{the-diagnostics-class}
...
\subsection{Determination of temperature and
density}\label{determination-of-temperature-and-density}
...
\subsection{Diagnostic diagram combining various
atoms}\label{diagnostic-diagram-combining-various-atoms}
...
\{'[NeIII] 3869/15.6m': ('Ne3', 'L(3869)/L(156000)', 'RMS([E(156000), E(3869)])'), '[NII] 5755/6584': ('N2', 'L(5755)/L(6584)', 'RMS([E(6584), E(5755)])'), '[NeIII] 3930+/15.6m': ('Ne3', '(L(3869)+L(3968))/L(156000)', 'RMS([E(156000), E(3869)*L(3869)/(L(3869)+L(3968)), E(3968)*L(3968)/(L(3869)+L(3968))])'), '[OIII] 4363/5007': ('O3', 'L(4363)/L(5007)', 'RMS([E(5007), E(4363)])'), '[SIII] 6312/18.7m': ('S3', 'L(6312)/L(187000)', 'RMS([E(187000), E(6312)])'), '[SII] 4069/4076': ('S2', 'L(4069)/L(4076)', 'RMS([E(4069), E(4076)])'), '[SII] 4072+/6720+': ('S2', '(L(4069)+L(4076))/(L(6716)+L(6731))', 'RMS([E(6716)*L(6716)/(L(6716)+L(6731)), E(6731)*L(6731)/(L(6716)+L(6731)), E(4069)*L(4069)/(L(4069)+L(4076)), E(4076)*L(4076)/(L(4069)+L(4076))])'), '[OII] 3727+/7325+': ('O2', '(L(3726)+L(3729))/(B("7319A+")+B("7330A+"))', 'RMS([E(3726)*L(3726)/(L(3726)+L(3729)), E(3729)*L(3729)/(L(3726)+L(3729)),BE("7319A+")*B("7319A+")/(B("7319A+")+B("7330A+")),BE("7330A+")*B("7330A+")/(B("7319A+")+B("7330A+"))])'), '[OII] 3726/3729': ('O2', 'L(3726)/L(3729)', 'RMS([E(3729), E(3726)])'), '[NII] 5755/6548': ('N2', 'L(5755)/L(6548)', 'RMS([E(6548), E(5755)])'), '[SII] 6731/6716': ('S2', 'L(6731)/L(6716)', 'RMS([E(6716), E(6731)])'), '[NII] 5755/6584+': ('N2', 'L(5755)/(L(6548)+L(6584))', 'RMS([E(6548)*L(6548)/(L(6548)+L(6584)), E(6584)*L(6584)/(L(6584)+L(6548)), E(5755)])')\}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}158}]:} \PY{k}{print} \PY{n}{obs}\PY{o}{.}\PY{n}{getIntens}\PY{p}{(}\PY{p}{)}
\end{Verbatim}

    \begin{Verbatim}[commandchars=\\\{\}]
\{'S2\_6716A': array([ 1.22000003]), 'O2\_7330A+': array([ 5.17000008]), 'S3\_6312A': array([ 0.75999999]), 'Ne2\_12.8m': array([ 8.30000019]), 'O1\_6364A': array([ 0.54000002]), 'Ne3\_15.6m': array([ 34.09999847]), 'O1\_6300A': array([ 1.69000006]), 'Ne3\_3869A': array([ 18.89999962]), 'Ne3\_3968A': array([ 6.4000001]), 'N2\_6548A': array([ 6.84000015]), 'S3\_18.7m': array([ 10.]), 'O2\_3729A': array([ 18.60000038]), 'S2\_6731A': array([ 2.18000007]), 'N2\_6584A': array([ 19.]), 'O3\_5007A': array([ 435.08999634]), 'N2\_5755A': array([ 0.50999999]), 'H1r\_6563A': array([ 3.45000005]), 'S4\_10.5m': array([ 7.]), 'S2\_4076A': array([ 0.44999999]), 'O3\_4363A': array([ 4.36000013]), 'H1r\_4861A': array([ 100.]), 'Ar3\_7136A': array([ 4.90999985]), 'O2\_3726A': array([ 39.70000076]), 'S2\_4069A': array([ 0.85000002]), 'O2\_7319A+': array([ 6.53999996])\}

    \end{Verbatim}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}159}]:} \PY{o}{\PYZpc{}}\PY{k}{config} InlineBackend.figure\PYZus{}format = \PYZsq{}png\PYZsq{}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_340_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
the index of the observation to be used is given by \textbf{i\_obs}:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}160}]:} \PY{n}{diags}\PY{o}{.}\PY{n}{plot}\PY{p}{(}\PY{n}{emisgrids}\PY{p}{,} \PY{n}{obs}\PY{p}{,} \PY{n}{i\PYZus{}obs} \PY{o}{=} \PY{l+m+mi}{0}\PY{p}{)}
\end{Verbatim}

    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_342_0.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}161}]:} \PY{n}{diags}\PY{o}{.}\PY{n}{diags}
\end{Verbatim}

            \begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}161}]:} \{'[NII] 5755/6548': ('N2', 'L(5755)/L(6548)', 'RMS([E(6548), E(5755)])'),
           '[NII] 5755/6584': ('N2', 'L(5755)/L(6584)', 'RMS([E(6584), E(5755)])'),
           '[NII] 5755/6584+': ('N2',
...
\textbf{diags} object, using for example:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}162}]:} \PY{n}{diags}\PY{o}{.}\PY{n}{addClabel}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{[OIII] 4363/5007}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{[OIII]na}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
\end{Verbatim}

\section{Determination of elemental abundances using
ICFs}\label{determination-of-elemental-abundances-using-icfs}
...
abundance of a given element: \[X(elem) = X(ion) * icf,\] where
\(X(ion)\) may be the abundance of one single ion or the sum of several
ions of the element. These expressions are generally obtained
empirically or semiempirically, based on the results of photoionization
models or a comparison between the ionization potentials of different
ions. Most expressions have been devised for a specific kind of object
(e.g., PNe, HII regions, etc) and should not be applied to objects of a
different kind.
...
The following snippet illustrates how PyNeb manages icfs:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}163}]:} \PY{n}{atom\PYZus{}abun} \PY{o}{=} \PY{p}{\PYZob{}}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{O2}\PY{l+s+s1}{\PYZsq{}}\PY{p}{:} \PY{l+m+mf}{0.001}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{O3}\PY{l+s+s1}{\PYZsq{}}\PY{p}{:} \PY{l+m+mf}{0.002}\PY{p}{,} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Ne3}\PY{l+s+s1}{\PYZsq{}}\PY{p}{:} \PY{l+m+mf}{1.2e\PYZhy{}5}\PY{p}{\PYZcb{}}
...
\{'Ne': ['KB94\_A28.10', 'KH01\_4d', 'direct\_Ne.2356', 'direct\_Ne.345', 'S78\_265b', 'direct\_Ne.23', 'KB94\_A28.8', 'KB94\_A28.6', 'DIMS14\_20', 'DIMS14\_17c', 'TPP77\_15', 'PHCD07\_13', 'PHCD07\_12', 'PC69\_40', 'KB94\_A27', 'KB94\_A28.10b', 'Ial06\_19b', 'Ial06\_19c', 'Ial06\_19a', 'DIMS14\_17a', 'DIMS14\_17b', 'direct\_Ne.2345', 'direct\_Ne.235']\}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}164}]:} \PY{n}{elem\PYZus{}abun} \PY{o}{=} \PY{n}{icf}\PY{o}{.}\PY{n}{getElemAbundance}\PY{p}{(}\PY{n}{atom\PYZus{}abun}\PY{p}{,} \PY{n}{icf\PYZus{}list}\PY{o}{=}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{TPP77\PYZus{}15}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{p}{)} \PY{c+c1}{\PYZsh{} Computes the Ne abundance with the TPP06 recipe}
...
\{'TPP77\_15': 1.8e-05\}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}165}]:} \PY{n}{elem\PYZus{}abun} \PY{o}{=} \PY{n}{icf}\PY{o}{.}\PY{n}{getElemAbundance}\PY{p}{(}\PY{n}{elem\PYZus{}abun}\PY{p}{)}  \PY{c+c1}{\PYZsh{} performs all the possible element abundance computations given the input ionic abundance set and available icf  }
          \PY{k}{print} \PY{n}{elem\PYZus{}abun}
\end{Verbatim}
...
the icf collection and its source papers; e.g.:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}166}]:} \PY{k}{print} \PY{n}{icf}\PY{o}{.}\PY{n}{getAvailableICFs}\PY{p}{(}\PY{p}{)}
\end{Verbatim}
...
\{'Mg': ['direct\_Mg.45'], 'Ne': ['KB94\_A28.10', 'KH01\_4d', 'direct\_Ne.2356', 'direct\_Ne.345', 'S78\_265b', 'direct\_Ne.23', 'KB94\_A28.8', 'KB94\_A28.6', 'DIMS14\_20', 'DIMS14\_17c', 'TPP77\_15', 'PHCD07\_13', 'PHCD07\_12', 'PC69\_40', 'KB94\_A27', 'KB94\_A28.10b', 'Ial06\_19b', 'Ial06\_19c', 'Ial06\_19a', 'DIMS14\_17a', 'DIMS14\_17b', 'direct\_Ne.2345', 'direct\_Ne.235'], 'Cl': ['KH01\_4f', 'mGKA07-PTPR92\_p269', 'DIMS14\_32', 'GKA07\_1.p269', 'Ial06\_21a', 'DIMS14\_29', 'DIMS14\_29b', 'direct\_Cl.34', 'direct\_Cl.234', 'direct\_Cl.23', 'Ial06\_21c', 'Ial06\_21b'], 'C': ['DIMS14\_39', 'KB94\_A13.10', 'KB94\_A12', 'KB94\_A16', 'KB94\_A19', 'KB94\_A26', 'KB94\_A21', 'KB94\_A13.10b'], 'O': ['KH01\_4b', 'KB94\_A10b', 'direct\_O.2345', 'DIMS14\_12', 'KB94\_A8', 'KB94\_A10', 'Ial06\_16', 'Ial06\_17', 'direct\_O.23', 'KB94\_A6', 'direct\_O.234', 'TPP77\_13'], 'N': ['KH01\_4c', 'KB94\_A1.6', 'direct\_N.23', 'KB94\_A1.8', 'DIMS14\_14', 'direct\_N.2345', 'KB94\_A1.10', 'DIMS14\_14b', 'KB94\_A0', 'direct\_N.234', 'TPP77\_14', 'Ial06\_18a', 'Ial06\_18c', 'Ial06\_18b'], 'S': ['KH01\_4e', 'direct\_S.2345', 'direct\_S.23', 'DIMS14\_26', 'DIMS14\_23', 'KB94\_A36.8', 'KB94\_A36.6', 'KB94\_A36.10b', 'Ial06\_20a', 'Ial06\_20b', 'Ial06\_20c', 'direct\_S.234', 'KB94\_A36.10'], 'Ar': ['Ial06\_22a', 'Ial06\_22b', 'Ial06\_22c', 'KH01\_4g', 'KH01\_4txt', 'DIMS14\_36', 'Ial06\_23c', 'Ial06\_23a', 'KB94\_A30.0', 'ITL94\_20', 'KB94\_A30.10', 'direct\_Ar.345', 'ITL94\_19', 'PHCD07\_17', 'PHCD07\_16', 'KB94\_A30.10b', 'direct\_Ar.23', 'direct\_Ar.234', 'KB94\_A32', 'DIMS14\_35', 'Ial06\_23b'], 'Fe': ['Ial06\_24b', 'Ial06\_24c', 'RR05\_2', 'RR05\_3', 'RR05\_4', 'Ial06\_24a'], 'He': ['KH01\_4a', 'direct\_He.23', 'DIMS14\_10', 'PTPR92\_21']\}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}167}]:} \PY{k}{print} \PY{n}{icf}\PY{o}{.}\PY{n}{getExpression}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{KH01\PYZus{}4g}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}   \PY{c+c1}{\PYZsh{} returns the analytical expression of the icf identified by the label KH01\PYZus{}4g}
\end{Verbatim}
...
Ar = (Ar3 + Ar4) * (He2 + He3) / He2 / (1 -  N2 / (KH01\_4c))
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}168}]:} \PY{k}{print} \PY{n}{icf}\PY{o}{.}\PY{n}{getReference}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{KH01\PYZus{}4g}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}   \PY{c+c1}{\PYZsh{} returns the bibliographic reference of the source paper}
\end{Verbatim}
...
Kwitter \& Henry 2001, ApJ, 562, 804
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}169}]:} \PY{k}{print} \PY{n}{icf}\PY{o}{.}\PY{n}{getURL}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{KH01\PYZus{}4g}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}   \PY{c+c1}{\PYZsh{} returns the ADS URL of the source paper}
\end{Verbatim}

    \begin{Verbatim}[commandchars=\\\{\}]
http://adsabs.harvard.edu/abs/2001ApJ{\ldots}562..804K
...
\section{The logging facility}\label{the-logging-facility}
...
\section{Simple diagnostic diagram}\label{simple-diagnostic-diagram}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}170}]:} \PY{o}{\PYZpc{}\PYZpc{}}\PY{k}{writefile} smc24.dat
          NAME SMC\PYZus{}24
          cHbeta     0.047
          S4\PYZus{}10.5m   7.00000
          Ne2\PYZus{}12.8m  8.3000
...
Overwriting smc24.dat
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}171}]:} \PY{c+c1}{\PYZsh{} Diagnostic plot}
...
warng EmissionLine: line 33.6m for atom S3 not valid
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_364_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\section{Extinction}\label{extinction}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}172}]:} \PY{c+c1}{\PYZsh{} Sample extinction in PyNeb}
...
<matplotlib.figure.Figure at 0x115787950>
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_366_1.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_366_2.pdf}
    \end{center}
    { \hspace*{\fill} \\}
...
\section{Line ratio and diagnostics}\label{line-ratio-and-diagnostics}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}173}]:} \PY{l+s+sd}{\PYZdq{}\PYZdq{}\PYZdq{}}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_368_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
...
\section{Contour plots}\label{contour-plots}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}174}]:} \PY{c+c1}{\PYZsh{} Plot the contour diagram of the selected diagnostics}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_370_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\section{Diagnostic diagrams of a 2-components
region}\label{diagnostic-diagrams-of-a-2-components-region}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}175}]:} \PY{c+c1}{\PYZsh{} Analysis plot of a simple two\PYZhy{}component model, meant to illustrate the bias arising }
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
0.89220155323 <type 'numpy.float64'>
2.30324282866 <type 'numpy.float64'>
0.899354220227 <type 'numpy.float64'>

    \end{Verbatim}

    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_372_1.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Diagnostic diagrams of a 2-components
region¶}\label{diagnostic-diagrams-of-a-2-components-region}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}176}]:} \PY{c+c1}{\PYZsh{} Emissivity plot}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_374_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Grotrian diagrams}\label{grotrian-diagrams}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}177}]:} \PY{c+c1}{\PYZsh{} Grotrian diagram of [O II]}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_376_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Comparison of transition
probabilities}\label{comparison-of-transition-probabilities}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}178}]:} \PY{c+c1}{\PYZsh{} Transition probabilities plot}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_378_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Comparison of collision strengths for a given
ion}\label{comparison-of-collision-strengths-for-a-given-ion}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}179}]:} \PY{c+c1}{\PYZsh{} Collision strength plot}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_380_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Emissivity map of a line}\label{emissivity-map-of-a-line}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}180}]:} \PY{c+c1}{\PYZsh{} Emissivity map of [O III] 5007 }
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_382_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
    
\chapter{Emissivity map of a line
ratio}\label{emissivity-map-of-a-line-ratio}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}181}]:} \PY{c+c1}{\PYZsh{} Emissivity map of the [O III] 4363/5007 line ratio}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{PyNeb_Handbook_files/PyNeb_Handbook_384_0.pdf}
    \end{center}
    { \hspace*{\fill} \\}
